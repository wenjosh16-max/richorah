...
interface DemandeVisiteFormProps {
  bienId: string
  bienTitre: string
}
...
export default function DemandeVisiteForm({ bienId }: DemandeVisiteFormProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!selectedDate || !selectedHeure) { alert("Veuillez choisir une date et un créneau"); return }
    setLoading(true)
    try {
      const res = await fetch("/api/visites", {
        ...
        body: JSON.stringify({
          bienId,
          ...formData,
          creneau: `${selectedDate}-${selectedHeure}`,
          frais,
        }),
      })
      const data = await res.json()
  ...
  if (step === "confirm") {
    return (
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 text-center">
        ...
        <p className="text-sm text-gray-500 mt-1">Votre code de visite unique :</p>
        ...
        <p className="text-xs text-gray-400 mt-3">
          Un agent vous contactera sur <strong>{formData.telClient}</strong> pour confirmer.
          <br />Frais de visite : <strong>{frais.toLocaleString()} FCFA</strong>
        </p>
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
      <div className="bg-gradient-to-r from-primary/5 via-primary/10 to-primary/5 p-6 border-b border-gray-100">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">Choisissez votre jour et créneau horaire</p>
          </div>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="border-t border-gray-100 pt-5 space-y-4">
            <div>
              ...
              <Input required minLength={2} value={formData.nomClient}
                onChange={(e) => setFormData({ ...formData, nomClient: e.target.value })}
                placeholder="Votre nom" className="mt-1" />
            </div>
            <div>
              <Label>Téléphone *</Label>
              <Input required type="tel" minLength={8} value={formData.telClient}
                onChange={(e) => setFormData({ ...formData, telClient: e.target.value })}
                placeholder="70 00 00 00" className="mt-1" />
            </div>
            <div>
              <Label>Email (optionnel)</Label>
              <Input type="email" value={formData.emailClient}
            ...
            <div>
              ...
              <textarea value={formData.message}
                ...
                placeholder="Ex: Je confirme le créneau choisi"
                rows={2}
      ...
      <div className="bg-gray-50 px-6 py-3 border-t border-gray-100">
        <p className="text-xs text-gray-400 text-center">
          Une question ? Appelez-nous au <strong className="text-gray-600">70 62 86 96</strong>
        </p>
      </div>
    </div>
  )
